...
class _AddContactViewState extends State<AddContactView> {
  final _formKey = GlobalKey<FormState>();//Clé unique pour identifier et contrôler l'état du formulaire
  final _name = TextEditingController();
  final _email = TextEditingController();
  final _phone = TextEditingController();
  bool _isLoading = false; // État local

  @override
  void dispose() {// Libère les ressources des contrôleurs pour éviter les fuites de mémoire
    _name.dispose();
    _email.dispose();
    _phone.dispose();
    super.dispose();
  ...
  Future<void> _addContact() async {
    ...
    setState(() => _isLoading = true); // rebuild ici
    ...
    try {
      ...
      if (mounted) {
        setState(() => _isLoading = false); // REBUILD ICI
      }
    }
  }
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: _isLoading ? null : _addContact,//si chargement le boutton est desactivé snn il est activé et lié à addContact
                      icon: _isLoading
                          ? const SizedBox(
                              width: 20,
                              height: 20,
